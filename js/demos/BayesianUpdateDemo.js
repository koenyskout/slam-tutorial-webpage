...
export class BayesianUpdateDemo {
  ...
  measurementFromTruth() {
    const dx = this.truthPose.x - this.landmark.x;
    const dy = this.truthPose.y - this.landmark.y;
    return {
  ...
  getHandles() {
    ...
    return [
      { kind: "truthPose", x: this.truthPose.x, y: this.truthPose.y, color: COLORS.truth },
      { kind: "priorMean", x: this.prior.x, y: this.prior.y, color: COLORS.prior },
      { kind: "priorSigma", x: this.prior.x + this.prior.sigma, y: this.prior.y, color: COLORS.prior },
      { kind: "landmark", x: this.landmark.x, y: this.landmark.y, color: COLORS.landmark },
      {
        kind: "measSigmaR",
        x: this.landmark.x + (obs.range + this.measurement.sigmaR) * Math.cos(obs.angle),
        y: this.landmark.y + (obs.range + this.measurement.sigmaR) * Math.sin(obs.angle),
        color: COLORS.meas,
      },
      {
        kind: "measSigmaA",
        x: this.landmark.x + obs.range * Math.cos(obs.angle + this.measurement.sigmaA),
        y: this.landmark.y + obs.range * Math.sin(obs.angle + this.measurement.sigmaA),
        color: COLORS.meas,
      },
    ];
  }
  ...
  onPointerMove(event) {
    ...
    if (this.dragMode === "truthPose") {
      this.truthPose.x = clamp(p.x, 6, 94);
      this.truthPose.y = clamp(p.y, 6, 94);
      return;
    }
    ...
    if (this.dragMode === "measSigmaR") {
      ...
      this.measurement.sigmaR = clamp(Math.abs(d - obs.range), 0.8, 13);
      return;
    }
    ...
    if (this.dragMode === "measSigmaA") {
      ...
      this.measurement.sigmaA = clamp(Math.abs(wrapAngle(a - obs.angle)), 0.05, 1.15);
    }
  }
  ...
  measurementLikelihood(x, y) {
    ...
    const dr = (r - obs.range) / this.measurement.sigmaR;
    const da = wrapAngle(a - obs.angle) / this.measurement.sigmaA;
  ...
  drawPriorCloud(mapper) {
    this.ctx.fillStyle = "rgba(141, 148, 156, 0.2)";
  ...
  drawMeasurementCloud(mapper) {
    ...
    for (const s of this.polarSamples) {
      const r = obs.range + s.gx * this.measurement.sigmaR;
      if (r < 0.5) continue;
      const a = obs.angle + s.gy * this.measurement.sigmaA;
  ...
  drawMeasurementGeometry(mapper) {
    ...
    const outer = r + sr;
    const a0 = obs.angle - sa;
    const a1 = obs.angle + sa;
    ...
    this.ctx.lineTo(
      mapper.toX(this.landmark.x + rayLength * Math.cos(obs.angle - sa)),
      mapper.toY(this.landmark.y + rayLength * Math.sin(obs.angle - sa)),
    );
    this.ctx.moveTo(mapper.toX(this.landmark.x), mapper.toY(this.landmark.y));
    this.ctx.lineTo(
      mapper.toX(this.landmark.x + rayLength * Math.cos(obs.angle + sa)),
  ...
  drawLegend(mapper) {
    ...
    this.ctx.fillText("Ground truth pose (blue)", x, y);
    this.ctx.fillStyle = COLORS.prior;
    this.ctx.fillText("Prior belief (gray)", x, y + 16);
    this.ctx.fillStyle = "#a17034";
    this.ctx.fillText("Range-bearing likelihood (orange)", x, y + 32);
  ...
  draw() {
    ...
    this.ctx.strokeStyle = "rgba(141,148,156,0.72)";
    ...
    this.ctx.strokeStyle = "rgba(95,134,165,0.42)";
    this.ctx.setLineDash([4, 4]);
    this.ctx.beginPath();
    this.ctx.moveTo(mapper.toX(this.prior.x), mapper.toY(this.prior.y));
    this.ctx.lineTo(mapper.toX(truth.x), mapper.toY(truth.y));
    this.ctx.moveTo(mapper.toX(posterior.meanX), mapper.toY(posterior.meanY));
    this.ctx.lineTo(mapper.toX(truth.x), mapper.toY(truth.y));
    this.ctx.stroke();
    this.ctx.setLineDash([]);

    this.drawCenter(mapper, this.landmark, COLORS.landmark, "landmark");
    this.drawCenter(mapper, truth, COLORS.truth, "truth");
    this.drawCenter(mapper, this.prior, COLORS.prior, "prior");
    this.drawCenter(mapper, { x: posterior.meanX, y: posterior.meanY }, COLORS.posterior, "post");
    ...
    const effectiveGain = priorToTruth > 1e-5 ? priorToPost / priorToTruth : 0;
    const priorErr = Math.hypot(this.prior.x - truth.x, this.prior.y - truth.y);
